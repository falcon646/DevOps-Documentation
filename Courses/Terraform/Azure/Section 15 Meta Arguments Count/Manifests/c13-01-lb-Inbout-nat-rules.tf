...
resource "azurerm_lb_nat_rule" "web_ib_inbound-nat_rule_22" {
  ...
  name = "${azurerm_lb.web_lb.name}-vm-${count.index}-inboud-nat-rule_${var.web_lb_inbound_nat_ports[count.index]}-22-ssh"
  protocol = "Tcp"
  frontend_port = var.web_lb_inbound_nat_ports[count.index]
  backend_port =  22
